...
- [x] T035 [US2] Create WithdrawalFlow component in `frontend/src/components/WithdrawalFlow.jsx`: implement multi-step wizard with steps: (1) EnterAmount (pre-filled with full balance, with validation: positive, max 2 decimals, <= balance) → (2) SelectMethod → (3) Confirm → (4) Success. Use local component state for current step, selected method, amount, loading, and error. Call api.createWithdrawal on confirm. Handle API errors (show insufficient balance message from error.details). On success, trigger context refetch. UI text in Spanish per Figma design
